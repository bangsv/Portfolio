...
def gradient(f, x, h=1e-05):
    ...
    return grad
def gradient_descent(f, x0, mu=0.05, h=1e-05, s=5000): # mu это шаг при равном 1 градиенте
    x = copy(x0)
    for _ in range(s):
        gfx = gradient(f,x,h)
        mgfx = scalar_multiply(mu*sqrt(sum(gi**2 for gi in gfx)),gfx)
        x = vector_add(x,  scalar_multiply(-1, mgfx))
    return x
...
def the_first_priciple_comp(data):
    ...
    w = gradient_descent(negate(partial(dir_variance,data)), init_w)
    return direction(w)
